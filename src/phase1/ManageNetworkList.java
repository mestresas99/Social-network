...
public class ManageNetworkList implements IManageNetworkList{
	...
	public StudentsList union(StudentsList lst1, StudentsList lst2) {
		...
		int i=0;
		while(i!=lst1.size) {
			resultList.addFirst(lst1.getAt(i));
			}
		while(i!=lst2.size) {
			resultList.addFirst(lst2.getAt(i));
			}
		return resultList;
	}
	...
	public StudentsList getCampusCity(StudentsList lst, int opc) {
		...
		if(opc==1){
			...
			if (node.elem.campus.toString().toLowerCase().equals(node.elem.city.toLowerCase())) {
		 ...
		 System.out.println("The option choosen is not avialable");
	 ...
	 * 4. This method takes a social network as input and a integer parameter opc so that: 
			...
			- If opc=2, the method returns a list of students sorted by descending
			order according to their full name. 
	...
	public static void sortedInsert(StudentsList lst, Student newStudent, int opc) {
		if (opc==1) {
		int i, j, aux;
		DNode node = lst.header.next;
        for (i = 0; i < lst.size - 1; i++) {
            for (j = 0; j < lst.size - i - 1; j++) {
                if (node.elem.email[j + 1] < node.elem.email[j]) {
                    aux = node.elem.email[j + 1];
                    node.elem.email[j + 1] = node.elem.email[j];
                    node.elem.email[j] = aux;
                }
            }
        	}	
			}
	...
	public StudentsList locateByCity(StudentsList lst, String city) {
		...
		DNode node = lst.header.next;
		int i =0;
		while(i!=lst.size && node.elem!=null) {
			node = node.next;
	...
	public StudentsList getStudentsByDateInterval(StudentsList lst, LocalDate start, LocalDate end) {
		StudentsList resultList=new StudentsList();
		//dates input (year,month, day)
		StudentsList l = new StudentsList();
		DNode node = lst.header.next;
		int i =0;
		while(i!=lst.size && node.elem!=null) {
			node = node.next;
			if(node.elem.date_sign_in.isAfter(start) && node.elem.date_sign_in.isBefore(end)) {
				l.addFirst(node.elem);
			}
		}
		return resultList;
